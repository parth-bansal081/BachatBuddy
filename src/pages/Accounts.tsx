...
const AccountsContent = () => {
  ...
  const queryClient = useQueryClient();
    ...
    // Return raw budget data — spent will be computed via useMemo below
    const { data: rawBudgets = [] } = useQuery({
      ...
      queryFn: async () => {
        ...
        return (data || []).map((b: any) => ({
          category: b.category,
          budget: Number(b.expected_amount || 0), // ✅ correct column name
          spent: 0,
        }));
      },
    });
  ...
  // Compute real spent per category from transactions (negative amounts = expenses)
