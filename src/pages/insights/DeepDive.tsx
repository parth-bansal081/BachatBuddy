...
import { Sparkles, Loader2, Brain, ShieldCheck } from "lucide-react";
import {
    AreaChart,
    Area,
    XAxis,
...
export default function InsightsDeepDive() {
    ...
    // Fetch AI Analysis on Mount/Update
    useEffect(() => {
        if (transactions.length > 0 && !aiSummary) {
            const fetchAIAdvice = async () => {
                setIsAiLoading(true);
                const advice = await aiService.analyzeFinance(transactions);
                setAiSummary(advice);
    ...
    return (
        <div className="space-y-8 max-w-7xl mx-auto p-4 md:p-8 animate-fade-in pb-24">
            ...
            {/* AI Strategic Advice Hero */}
            <Card className="border-0 bg-gradient-to-r from-primary/20 via-primary/5 to-accent/10 shadow-2xl overflow-hidden relative group">
                ...
                <CardContent className="relative z-10">
                    {isAiLoading ? (
                        <div className="flex items-center gap-3 p-4">
                            ...
                            <p className="text-lg font-medium italic text-muted-foreground">My agents are performing a forensic audit of your transactions...</p>
                        </div>
                    ) : (
                        <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 shadow-inner">
                            <p className="text-xl font-medium leading-relaxed italic text-foreground tracking-tight">
                                "{aiSummary || "Awaiting transaction data to generate your strategic summary..."}"
                            </p>
                            <div className="mt-4 flex gap-4 text-[10px] items-center text-muted-foreground uppercase tracking-widest">
                                <span className="flex items-center gap-1"><Brain className="h-3 w-3" /> Core AI Engaged</span>
                                <span className="flex items-center gap-1"><ShieldCheck className="h-3 w-3" /> Auditor Validated</span>
                            </div>
                        </div>
                    )}
                </CardContent>
