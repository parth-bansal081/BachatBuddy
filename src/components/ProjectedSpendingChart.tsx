...
export const SpendingTrendsChart = ({ data, currencySymbol = "₹", className = "" }: SpendingTrendsChartProps) => {
  ...
  // Safe mock-data baseline fallback array to prevent crashes when arrays are empty
  ...
  return (
    <div className={`w-full h-full flex flex-col justify-between ${className}`}>
      ...
      {/* Primary SVG Rendering Canvas Region */}
      <div className="flex-1 w-full min-h-0 relative">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            {/* ⚡ THE FIXED ANTI-OVERLAP CUSTOM TOOLTIP PORTAL */}
            <Tooltip
              allowEscapeViewBox={{ x: true, y: true }}
              content={({ active, payload, label, coordinate }) => {
                if (active && payload && payload.length) {
                  // Calculate coordinates dynamically relative to pointer positioning context
